...
def redirectme():
    ...
    # sed
    subprocess.check_call(r"sed -i 's#^command=/usr/bin/Xvfb.*$#command=/usr/bin/Xvfb :1 -screen 0 {width}x{height}x24#' /etc/supervisor/conf.d/supervisord.conf".format(**env),
                          shell=True)
